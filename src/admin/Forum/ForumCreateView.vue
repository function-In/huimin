...
<template>
    <div class="forumUpdate">
        <div class="user_header_back"
            style="display: flex;flex-direction: row;justify-content: space-between;align-items: center;">
            <div>
                <el-button type="default" @click="RouterBack()" style="margin-right: 10px;">返回</el-button>发布帖子
            </div>
            <el-form-item>
...
const beforeRemove = (file) => {
    if (!file.raw) {
        Axios.Post(undefined,
            [{ name: 'meta', value: 'dorum_image_delete_require' }, { name: 'id', value: file.id }],
            (result) => {
...
Axios.Post(undefined,
    ...
    (result) => {
        if (result.data.code == 200) {
            ...
            table.value.typeid = result.data.data[0].id;
        } else notify('info', result.data.code + '', result.data.info);
    },
